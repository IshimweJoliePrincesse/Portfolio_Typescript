...
/**
 * Hides the navbar while scrollong down
 * @param {Object} config
 * @param {String} [config.id=navbar] -id of navabr
    ...
    >
      <h1 className="relative text-2xl capitalize font-signature text-accent group top-1">
        <Link href="/#hero" className="block">
          {author.name}
          <div className="absolute bottom-1.5 left-0 h-[1px] w-0 group-hover:full bg-accent duration-300"></div>
        </Link>
      </h1>
      <NavButton
        onClick={() => {
