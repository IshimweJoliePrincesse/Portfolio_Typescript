...
//for separaring anmation props from button props
...
const Button=(props:Props & MotionProps)=>{
    ...
    if(props.type==='link'){
        const {sameTab, ...motiopnProps}=props;
        removeKeys<Props & LinkProps>(motiopnProps, buttonProps);

        return (
            <motion.span {...motiopnProps}>
                <Link
                className={classes}
                href={props.href}
    ...
    if (type==='button'){
        return (
            <button type={type} className={classes} onClick={props.onClick}>{children}</button>
        )
    }
